<?php 
                    ...
                    <form role="form" class="form-horizontal" action="modules/ciudad/proses.php?act=insert" method="POST">
                        <div class="box-body">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-sm-5">
                                    <input type="text" class="form-control" name="descrip_ciudad" pleaceholder="Ingresa una ciudad" required>
                                </div>
                            </div>
    ...
    elseif($_GET['form']=='edit'){
        if (isset($_GET['id'])) {
            // Consultar los datos de la ciudad junto con la descripción del departamento
            $query = mysqli_query($mysqli, "SELECT ciudad.*, departamento.dep_descripcion 
                                            FROM ciudad 
                                            JOIN departamento ON ciudad.id_departamento = departamento.id_departamento 
                                        ...
                                        <?php 
                                           ...
                                           if (mysqli_num_rows($query) > 0) {
                                               while ($data2 = mysqli_fetch_assoc($query)) {
                                                   // Acceder y mostrar la descripción correctamente
                                                   $depDescripcion = isset($data2['dep_descripcion']) ? htmlspecialchars($data2['dep_descripcion']) : 'Descripción no disponible';
                                                   echo "<option value='" . htmlspecialchars($data2['id_departamento']) . "' $selected>";
                                                   echo $depDescripcion;
                                                   echo "</option>";
                                               }
                                           } else {
                                               echo "No se encontraron departamentos en la base de datos.";
                                           }
